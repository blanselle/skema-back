 ...
 */
class SchoolReportVoter extends Voter
{
    ...
    protected function supports(string $attribute, mixed $subject): bool
    {
        ...
        if($subject instanceof SchoolReport && $attribute === self::ACTION_CREATE) {
            return true;
        }
        ...
        if(!$subject['original'] instanceof SchoolReport) {
            return false;
        };

        if(!$subject['object'] instanceof SchoolReport) {
            return false;
        };
    ...
    protected function voteOnAttribute(string $attribute, mixed $subject, TokenInterface $token): bool
    {
        ...
        $this->logger->error('SchoolReportVoter: Invalid attribute', [
            'attribute' => $attribute,
            'subject' => $subject,
        ]);
    ...
    {
        if((new DateTime()) <= $this->getDateFinCV($student) or (new DateTime()) <= $this->getDate($student)) {
            return true;
        }
    ...
    private function canEdit(SchoolReport $originalSchoolReport, SchoolReport $updatedSchoolReport, Student $student): bool
    {
        if((new DateTime()) <= $this->getDateFinCV($student) and (new DateTime()) <= $this->getDate($student)){
            return true;
        }

        /** @var PropertyAccessor $propertyAccessor */
        $propertyAccessor = PropertyAccess::createPropertyAccessor();
        $class = new ReflectionClass(SchoolReport::class);

        foreach($class->getProperties() as $property) {
            $field = $property->getName();

            $originalValue = $propertyAccessor->getValue($originalSchoolReport, $field);
            $newValue = $propertyAccessor->getValue($updatedSchoolReport, $field);
