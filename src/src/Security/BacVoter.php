...
class BacVoter extends Voter
{
    ...
    protected function supports(string $attribute, mixed $subject): bool
    {
        ...
        if($subject instanceof Bac && $attribute === self::ACTION_CREATE) {
            return true;
        }
        ...
        if(!$subject['original'] instanceof Bac) {
            return false;
        };

        if(!$subject['object'] instanceof Bac) {
            return false;
        };
    ...
    protected function voteOnAttribute(string $attribute, mixed $subject, TokenInterface $token): bool
    {
        ...
        $this->logger->error('BacVoter: Invalid attribute', [
            'attribute' => $attribute,
            'subject' => $subject,
        ]);
    ...
    private function canEdit(Bac $originalBac, Bac $updatedBac, Student $student): bool
    {
        if((new DateTime()) <= $this->getDateFinCV($student)) {
            return true;
        }
        ...
        $class = new ReflectionClass(Bac::class);

        foreach($class->getProperties() as $property) {
            $field = $property->getName();
            
            $originalValue = $propertyAccessor->getValue($originalBac, $field);
            $newValue = $propertyAccessor->getValue($updatedBac, $field);
